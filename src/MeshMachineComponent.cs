...
{
    public class MeshMachineComponent : GH_Component
    {
        /// <summary>
        ...
        {
            // 0-geometry,1-length,2-FixC,3-FixV,4-Flip,5-pull,6-curvat,7-Reset
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            //DA.GetData<double>(5, ref FixT);
            DA.GetData<double>(5, ref PullStrength);
            //DA.GetData<double>(7, ref SmoothStrength);
            //DA.GetData<int>(8, ref Smoothweight);
            //DA.GetData<double>(9, ref LengthTol);
            //DA.GetData<bool>(5, ref Minim);
            DA.GetData<double>(6, ref CurvDep);
            //DA.GetData<double>(11, ref Blend);
                    ...
                    if (P.Halfedges[2 * i].IsUnused == false)
                    {
                          ...
                          && (Visited[vEnd] == false))
                        {
                            ...
                            if (CurvDep>0)
                            {
                                ...
                                //if (Blend != 0)
                                //{
                                //    double NeighbourEdgeLengths = 0;
                                //    int edgecount = 0;
                                //    int[] outgoing = P.Vertices.GetHalfedges(vStart);
                                //    for (int j = 0; j < outgoing.Length; j++)
                                //    {
                                //        NeighbourEdgeLengths += P.Halfedges.GetLength(outgoing[j]);
                                //        edgecount++;
                                //    }
                                //    outgoing = P.Vertices.GetHalfedges(vEnd);
                                //    for (int j = 0; j < outgoing.Length; j++)
                                //    {
                                //        NeighbourEdgeLengths += P.Halfedges.GetLength(outgoing[j]);
                                //        edgecount++;
                                //    }
                                //    NeighbourEdgeLengths *= 1.0 / edgecount;
                                //    L2 = (L2 * (1 - Blend) + NeighbourEdgeLengths * Blend);
                                //}
                            }
        ...
        {
            get { return new Guid("{76303274-a3ae-4ec1-9e9a-2ae3adeac246}"); }
        }
